...
public class JarAsm {
	...
	public static void main(String[] args) throws Exception {
		...
		// }
		System.out
				.println("--------------- Start Instrumentation Byte Code ---------------");
...
//
//		if (!file.isDirectory()) {
//			System.out.println(rootPath + "/" + packageName
//					+ " is not directory");
//			return;
//		}
//		RealPath = rootPath;
...
//
//		byte[] b = CpuDump$1Dump.dump(args[2]);

		// new File(rootPath +
		// "/org/cerberus/profile/cpu/CpuDump$1.class").delete();

//		FileOutputStream fos = new FileOutputStream(new File(rootPath
//				+ "/org/cerberus/profile/cpu/CpuDump$1.class"));
//		fos.write(b);
//		fos.close();

		new File(rootPath + "/org/cerberus/cccc").mkdirs();

		scanDirectory(file.getAbsolutePath());

		System.out
				.println("--------------- finish Instrumentation Byte Code ---------------");
	...
	private static void scanDirectory(String path) throws Exception {
		...
		for (String list : rootDirectory.list()) {
			...
			File childFile = new File(rootDirectory.getAbsolutePath() + "/"
					+ list);
	...
	private static void scanFile(String path) throws Exception {
		...
		try {
			...
			// is class file
//			if (path.indexOf("android") > 0 && path.indexOf("support") > 0
//					&& path.indexOf("v4") > 0) {
//				return;
//			}
//			if (path.indexOf("android") > 0 && path.indexOf("support") > 0
...
//					&& path.indexOf("actionbarsherlock") > 0) {
//				return;
//			}
